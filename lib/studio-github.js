...
function config() {
  ...
  const repository = process.env.GITHUB_CONTENT_REPOSITORY;

  if (!token) throw new Error("GITHUB_CONTENT_TOKEN이 설정되지 않았습니다.");
  if (!repository) throw new Error("GITHUB_CONTENT_REPOSITORY가 설정되지 않았습니다.");
...
export function githubBlobPayload(file) {
  return {
...
export function requiredFileAction(file, fallbackAction) {
  const action = file.action || fallbackAction;
  if (action !== "create" && action !== "update" && action !== "upsert") {
    throw new Error("GitHub 파일 저장 동작이 올바르지 않습니다.");
  }
  return action;
}
...
async function commitFiles({ title, files, action }) {
  for (const file of files) {
    ...
    const fileAction = requiredFileAction(file, action);
    if (fileAction === "create" && exists) {
      throw new Error(`${file.path} 파일이 이미 존재합니다.`);
    }
    if (fileAction === "update" && !exists) {
  ...
  const blobs = await Promise.all(
    files.map((file) =>
      github("/git/blobs", {
        method: "POST",
        body: JSON.stringify(githubBlobPayload(file)),
      }),
    ),
  );
