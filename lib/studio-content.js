...
import { validateStudioAssets } from "./studio-assets.js";
...
export function buildStudioPostFiles(draft) {
  ...
  const assetFiles = validateStudioAssets(draft.assets, {
    original: draft.original,
    revised: draft.revised,
  });

  const contentFiles = editing
    ? [{ path: publishedPath, content: publishedContent }]
    : [
        { path: originalPath, content: originalContent },
        { path: publishedPath, content: publishedContent },
      ];
  const files = [...contentFiles, ...assetFiles];
